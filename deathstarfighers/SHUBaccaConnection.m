...
//    });
    ...
    dispatch_async( backgroundQueue, ^(void) {
        ...
        dispatch_async(dispatch_get_main_queue(), ^{
            ...
            //connectionTimer = [NSTimer scheduledTimerWithTimeInterval:30.0 target:self selector:@selector(timerTicked:) userInfo:nil repeats:NO];
        });
    });
...
- (void)fetchSHUs {
    ...
    if ( data == nil ) {
        ...
        if ( error != Nil ) {
            ...
            NSLog( @"SHUBaccaConnection: Retrieved SHU JSON from the server: %lu shus to analyze", (unsigned long)[shuArray count] );
            ...
            for( NSMutableDictionary * shu in shuArray ) {
                ...
                NSString * shu_id_str = [[shu valueForKey:@"id"] description];
                NSInteger shu_id = [shu_id_str integerValue];
                
                NSString * title = [[shu_id_str stringByAppendingString:@": "] stringByAppendingString:[[shu valueForKey:@"description"] description]];
                ...
                [shus addObject:shu];
                NSInteger shu_index = [shus count] - 1;

                NSLog( @"SHUBaccaConnection: Found SHU %li stored at index %li", (long)shu_id, (long)shu_index );
...
- (NSMutableArray *)fetchLastStatusForId:(NSInteger)shu_id {
    ...
    NSLog( @"SHUBaccaConnection: Fetching Last status" );
    
    NSString * shu_id_str = [NSString stringWithFormat:@"%li", (long)shu_id];
    
    NSData * data = [NSData dataWithContentsOfURL:[NSURL URLWithString:shubaccaGetLastStatusForIDUrl(shu_id_str)]];
    
    if ( data == nil ) {
        NSLog( @"SHUBaccaConnection: Failed to get last status" );
    } else {
        NSArray * statusArray = [NSJSONSerialization JSONObjectWithData:data options:NSJSONReadingMutableContainers error:&error];
        
        if ( error != Nil ) {
            NSLog( @"SHUBaccaConnection: There was an error trying to retrieve the SHU last status from the server" );
        } else {
            NSLog( @"SHUBaccaConnection: Retrieved status from the server: %lu fields to analyze", (unsigned long)[statusArray count] );
            
            return [[NSMutableArray alloc] initWithArray:statusArray];
...
//            NSLog( @"SHUBaccaConnection: There was an error trying to retrieve the SHU JSON from the server" );
//        } else {
//            NSLog( @"SHUBaccaConnection: Retrieved Config from the server: %lu fields to analyze", (unsigned long)[configArray count] );
//            
//            
